...
// getHostInfoJSON Assuming that the /opt/trustagent/var/system-info/platform-info file has been created
// during startup, this function reads the contents of the json file and returns its bytes.
func getHostInfoJSON() ([]byte, error) {

	if _, err := os.Stat(constants.PlatformInfoFilePath); os.IsNotExist(err) {
		log.WithError(err).Errorf("common/hostinfo:getHostInfoJSON() %s - %s does not exist", message.AppRuntimeErr, constants.PlatformInfoFilePath)
		return nil, err
	}

	jsonData, err := ioutil.ReadFile(constants.PlatformInfoFilePath)
	if err != nil {
		log.Errorf("common/hostinfo:getHostInfoJSON() %s - There was an error reading %s", message.AppRuntimeErr, constants.PlatformInfoFilePath)
		return nil, err
	}
