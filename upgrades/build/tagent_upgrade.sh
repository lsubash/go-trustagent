...
if [ $? -ne 0 ]; then echo "Failed to create backup directory for default software manifests, exiting."; exit 1; fi
...
for filename in `ls $TRUSTAGENT_VAR_DIR/manifest_*.xml`; do
  if grep -q "Label=\"ISecL_Default" $filename; then
    ...
    if [ $? -ne 0 ]; then echo "Failed to take backup of default software manifests, exiting."; exit 1; fi
  fi
done
